...
server.route(
  {
    ...
    path: "/api/ssid",
    handler: function(request, reply) {
      console.log("/api/ssid", request.payload);
      let ret = [{ result: "failed" }, { result: "success", current: "ESPERT-3020", ip: "192.168.1.1" }];
      ...
      let list = [
        ...
        { ssid: "Baxter Windsor" }, { ssid: "Windsor Clinton" },
        { ssid: "Kurtis Barnaby Reginald" }, { ssid: "Nevil James Baz" },
        { ssid: "Bud Jemmy Paulie" }, { ssid: "Julian Nigellus Devin" }
      ];
      let idx = parseInt((Math.random() * 100)) % list.length;
      //let json = ret[Math.floor(Math.random(100) * 10) % 2];
      return reply(JSON.stringify(list));
    }
  });
